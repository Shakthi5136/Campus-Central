...
Date date2= new Date(); 
System.out.println(formatter1.format(date1)+formatter2.format(date2));  
...
<h2 style="margin-left:30px;">  List of latest college Event festivals  in Tamil Nadu in <%=formatter1.format(date1) %>. </h2><br>
       ...
       con=Dbconn.create();
       PreparedStatement ps=con.prepareStatement("SELECT * FROM `collegeevent`.`freeevent` where Etype='"+id+"' and ldate>='"+formatter2.format(date2)+"' and status='Approved'");
       PreparedStatement ps1=con.prepareStatement("SELECT count(*) FROM `collegeevent`.`freeevent` where Etype='"+id+"' and ldate>='"+formatter2.format(date2)+"' and status='Approved'");
