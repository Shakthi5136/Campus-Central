...
<%String staid=session.getAttribute("stafid").toString(); %>
  ...
  <thead>
    <tr>
      ...
      <th scope="col">STAFF ID</th>
       ...
       con=dbcon.create();
       PreparedStatement ps=con.prepareStatement("SELECT * FROM `college`.`chat` where status1=''and status2='student' and stafid='"+staid+"'");
...
<td><p class="card-text"><%=rs.getString(2)%></p></td>
       ...
       <td><a href="chatview.jsp?id=<%=rs.getString(1)%>&&sta=<%=rs.getString(2)%>&&stu=<%=rs.getString(3)%>"> Chat<i class='fa fa-trash-alt'></i></a></td>
